...
interface DocumentSetSyncOutboxRepository : JpaRepository<DocumentSetSyncOutboxEntity, Long> {
    ...
    @Query(
        """
            ...
            WHERE row.id = :id
              ...
              AND row.status = com.onyx.foss.kotlin.documentset.DocumentSetSyncStatus.IN_PROGRESS
        """,
    )
    fun renewOwned(
        @Param("id") id: Long,
    ...
    @Query(
        """
            UPDATE DocumentSetSyncOutboxEntity row
            SET row.status = com.onyx.foss.kotlin.documentset.DocumentSetSyncStatus.DONE,
                row.claimToken = NULL,
                row.lockedAt = NULL,
                row.lastError = NULL
            WHERE row.id = :id
    ...
    @Query(
        """
            UPDATE DocumentSetSyncOutboxEntity row
            SET row.status = com.onyx.foss.kotlin.documentset.DocumentSetSyncStatus.PENDING,
                row.claimToken = NULL,
                row.lockedAt = NULL,
