...
data class OpenSearchChunkDocument(
    ...
    fun toSpringAiDocument(id: String, score: Double? = null): Document {
        ...
        projectKey?.let { meta["project_key"] = it }
        repository?.let { meta["repository"] = it }
        space?.let { meta["space"] = it }
        status?.let { meta["status"] = it }
        documentType?.let { meta["document_type"] = it }
        searchContext?.let { meta["search_context"] = it }
